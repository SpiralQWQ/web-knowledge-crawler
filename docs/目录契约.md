...
├── shared/               # ★ Pure utility functions (no business logic)
...
| shared/ | pure functions (no business) | any business logic |
...
- **No reverse dependency**: shared must not import core; core must not import app
...
- ❌ No business logic scattered into `shared/` or `utils/` (pure functions only)
