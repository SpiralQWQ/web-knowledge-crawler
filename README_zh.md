...
- 整理进 `知识库/` 目录：`{词}/{站}/{类型}/00_日期_标题_大小/` + `meta.json`
...
| 🗂️ | 规范落盘 | `知识库/{词}/{站}/{类型}/00_日期_标题_大小/` + meta.json + HTML 双份 |
...
**Q：爬取的数据存在哪？什么格式？**
默认 `知识库/{词}/{站}/{类型}/00_日期_标题_大小/` + `meta.json`。可用 `--out-dir` 换位置。详见 `docs/输出规范.md`。
